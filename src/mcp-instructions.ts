...
function buildGeneralMcpInstructions(opts: McpInstructionsContext): string {
  const parts = [
    ...
    'Files / binary content: use download-bytes for any binary read (drive file content, mail attachments, profile photos, Teams hosted content, meeting recordings); pass it a Graph path or an absolute @microsoft.graph.downloadUrl from a metadata response. To keep a mail attachment as a file (PDF, DOCX, image) without the bytes passing through the client, use save-attachment-to-onedrive: it copies the attachment into OneDrive server-side and returns metadata only. For uploads, upload-file-content takes a base64 string body up to 4MB; use create-upload-session above that.',
  ];
